...
impl<F: ConnectionFactory> PostgresClient<F> {
    pub(crate) async fn establish(&mut self, settings: &PostgresConnectionSettings, channel_binding_data: Option<Vec<u8>>) -> Result<(), ElefantClientError> {
        self.connection
        ...
        match msg {
            ...
            BackendMessage::AuthenticationSASL(ref sasl) => {
                ...
                let supported_mechanism = select_sasl_mechanism(
                    &sasl.mechanisms,
                    channel_binding_data.is_some(),
                );

                let (channel_binding, mechanism_name) = match (&supported_mechanism, channel_binding_data) {
                    (Some(SaslMechanism::ScramSha256Plus), Some(data)) => {
                        (ChannelBinding::tls_server_end_point(data), sasl::SCRAM_SHA_256_PLUS)
                    }
...
fn select_sasl_mechanism(mechanisms: &[impl AsRef<str>], has_channel_binding: bool) -> Option<SaslMechanism> {
    if has_channel_binding {
        let has_plus = mechanisms.iter().any(|m| m.as_ref() == sasl::SCRAM_SHA_256_PLUS);
...
mod tls_tests {
    use crate::tokio_connection::{new_client, TokioPostgresPool, TokioConnectionFactory};
    ...
    async fn tls_prefer_uses_tls_when_available() {
        ...
        let settings = PostgresConnectionSettings {
            tls: TlsSettings::prefer(tls_config()),
            ..tls_settings_for_port(5603)
        };
        let mut client = new_client(settings).await.unwrap();
        let ssl_in_use: bool = client
            .read_single_value_simple("select ssl from pg_stat_ssl where pid = pg_backend_pid()")
            .await;
        assert!(ssl_in_use, "Connection should be using SSL in prefer mode when server supports it");
    }
