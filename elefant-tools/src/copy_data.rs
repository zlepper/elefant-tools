...
pub async fn copy_data<'d, S: CopySourceFactory, D: CopyDestinationFactory<'d>>(source: &S, destination: &'d mut D, options: CopyDataOptions) -> Result<()> {
    ...
    let destination_definition = if options.differential {
        destination.try_get_introspeciton().await?.unwrap_or_default()
    } else {
        default()
    };
    ...
    match &mut destination {
        SequentialOrParallel::Sequential(ref mut destination) => {
            apply_post_copy_structure_sequential(destination, &target_definition, &destination_definition).await?;
        }
        SequentialOrParallel::Parallel(ref mut destination) => {
            apply_post_copy_structure_parallel(destination, &target_definition, &options, &destination_definition).await?;
        }
    }
...
#[instrument(skip_all)]
async fn apply_pre_copy_structure<D: CopyDestination>(destination: &mut D, definition: &PostgresDatabase, target_definition: &PostgresDatabase) -> Result<()> {
    let identifier_quoter = destination.get_identifier_quoter();

    for schema in &definition.schemas {
        if target_definition.try_get_schema(&schema.name).is_some() {
            debug!("Schema {} already exists in destination", schema.name);
            continue;
        }
    ...
    for ext in &definition.enabled_extensions {
        if target_definition.enabled_extensions.iter().any(|e| e.name == ext.name) {
    ...
    for schema in &definition.schemas {
        ...
        for function in &schema.functions {
            if target_schema.is_some_and(|s| s.functions.iter().any(|f| f.function_name == function.function_name)) {
                debug!("Function {} already exists in destination", function.function_name);
                continue;
            }
        ...
        for aggregate_function in &schema.aggregate_functions {
            if target_schema.is_some_and(|s| s.aggregate_functions.iter().any(|f| f.function_name == aggregate_function.function_name)) {
                debug!("Aggregate function {} already exists in destination", aggregate_function.function_name);
                continue;
            }
        ...
        for table in &schema.tables {
            if target_schema.and_then(|s| s.try_get_table(&table.name)).is_some() {
...
#[instrument(skip_all)]
async fn do_copy<S: CopySource, D: CopyDestination>(source: &S, destination: &mut D, target_schema: &PostgresSchema, target_table: &PostgresTable, source_schema: &PostgresSchema, source_table: &PostgresTable, data_format: &DataFormat, options: &CopyDataOptions) -> Result<()> {
...
#[instrument(skip_all)]
fn get_post_apply_statement_groups(definition: &PostgresDatabase, identifier_quoter: &IdentifierQuoter, target_definition: &PostgresDatabase) -> Vec<Vec<String>> {
    ...
    for schema in &definition.schemas {
        ...
        for table in &schema.tables {
            ...
            for index in &table.indices {
                ...
                if existing_table.is_some_and(|t| t.indices.iter().any(|i| i.name == index.name)) {
                    debug!("Index {} on table {} already exists in destination", index.name, table.name);
                    continue;
                }
        ...
        for sequence in &schema.sequences {
            if existing_schema.is_some_and(|s| s.sequences.iter().any(|seq| seq.name == sequence.name)) {
        ...
        for table in &schema.tables {
            ...
            for column in &table.columns {
                let target_column = existing_table.and_then(|t| t.columns.iter().find(|c| c.name == column.name));

                if target_column.is_some_and(|c| c.default_value == column.default_value) {
                    debug!("Default value for column {} on table {} already matches destination", column.name, table.name);
                    continue;
                }
        ...
        for table in &schema.tables {
            ...
            for constraint in &table.constraints {
                if let PostgresConstraint::Unique(uk) = constraint {
                    if existing_table.is_some_and(|t| t.constraints.iter().any(|c| c.name() == constraint.name())) {
                        debug!("Unique constraint {} on table {} already exists in destination", constraint.name(), table.name);
                        continue;
                    }
    ...
    for schema in &definition.schemas {
        ...
        for table in &schema.tables {
            ...
            for constraint in &table.constraints {
                ...
                if existing_table.is_some_and(|t| t.constraints.iter().any(|c| c.name() == constraint.name())) {
                    debug!("Foreign key constraint {} on table {} already exists in destination", constraint.name(), table.name);
                    continue;
                }
    ...
    for schema in &definition.schemas {
        ...
        for trigger in &schema.triggers {
            ...
            if existing_schema.is_some_and(|s| s.triggers.iter().any(|t| t.name == trigger.name)) {
                debug!("Trigger {} on table {} already exists in destination", trigger.name, trigger.table_name);
                continue;
            }
    ...
    for job in &definition.timescale_support.user_defined_jobs {
        ...
        if target_definition.timescale_support.user_defined_jobs.iter().any(|j| j.function_schema == job.function_schema && j.function_name == job.function_name && j.config == job.config) {
            debug!("Timescale job {} already exists in destination", job.function_name);
            continue;
        }
    ...
    for schema in &definition.schemas {
        ...
        for table in &schema.tables {
            ...
            if let TableTypeDetails::TimescaleHypertable { compression: existing_compression, retention: existing_retention, ..} = &table.table_type {


                let existing_table = existing_schema.and_then(|s| s.try_get_table(&table.name));

                if existing_table.is_some_and(|t| {
                    if let TableTypeDetails::TimescaleHypertable {compression, retention, ..} = &t.table_type {
                        compression == existing_compression && retention == existing_retention
                    } else {
                        false
                    }
                }) {
                    debug!("Timescale hypertable {} already exists in destination", table.name);
                    continue;
                }
...
#[instrument(skip_all)]
async fn apply_post_copy_structure_sequential<D: CopyDestination>(destination: &mut D, definition: &PostgresDatabase, target_definition: &PostgresDatabase) -> Result<()> {
    let identifier_quoter = destination.get_identifier_quoter();

    let statement_groups = get_post_apply_statement_groups(definition, &identifier_quoter, target_definition);
...
#[instrument(skip_all)]
async fn apply_post_copy_structure_parallel<D: CopyDestination + Sync + Clone>(destination: &mut D, definition: &PostgresDatabase, options: &CopyDataOptions, target_definition: &PostgresDatabase) -> Result<()> {
    let identifier_quoter = destination.get_identifier_quoter();
