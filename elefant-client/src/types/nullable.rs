...
mod tests {
    ...
    mod tokio_connection {
        ...
        async fn test_nullable_types() {
            ...
            let value: Option<i16> = client.read_single_value("select value from test_table;", &[]).await.unwrap();
            assert_eq!(value, Some(42));

            client.execute_non_query("delete from test_table; insert into test_table values (null);", &[]).await.unwrap();

            let value: Option<i16> = client.read_single_value("select value from test_table;", &[]).await.unwrap();
            assert_eq!(value, None);

            let result = client.read_single_value::<i16>("select value from test_table;", &[]).await;
            ...
            client.execute_non_query("insert into test_table values ($1);", &[&None::<i16>]).await.unwrap();
            let value: Option<i16> = client.read_single_value("select value from test_table;", &[]).await.unwrap();
            assert_eq!(value, None);

            client.execute_non_query("delete from test_table;", &[]).await.unwrap();

            client.execute_non_query("insert into test_table values ($1);", &[&Some(42i16)]).await.unwrap();
            let value: Option<i16> = client.read_single_value("select value from test_table;", &[]).await.unwrap();
            assert_eq!(value, Some(42));
        }
    }
