...
use elefant_client::{QueryResultSet, PostgresConnectionSettings};
...
async fn test_monoio_basic_functionality() {
    ...
    // Test simple query
    let mut query_result = client.query("SELECT 42::int4 as answer", &[]).await.unwrap();
...
async fn test_monoio_table_operations() {
    ...
    client.query("DROP TABLE IF EXISTS monoio_integration_test", &[]).await.unwrap();
    client.query("CREATE TABLE monoio_integration_test (id INTEGER, data TEXT)", &[]).await.unwrap();
    client.query("INSERT INTO monoio_integration_test VALUES (1, 'hello'), (2, 'world')", &[]).await.unwrap();
    
    let mut query_result = client.query("SELECT id, data FROM monoio_integration_test ORDER BY id", &[]).await.unwrap();
    let result_set = query_result.next_result_set().await.unwrap();
    ...
    match result_set {
        QueryResultSet::RowDescriptionReceived(mut row_reader) => {
            ...
            assert_eq!(String::from_utf8(row1_data[1].unwrap().to_vec()).unwrap(), "hello");
            ...
            assert_eq!(String::from_utf8(row2_data[1].unwrap().to_vec()).unwrap(), "world");
    ...
    client.query("DROP TABLE monoio_integration_test", &[]).await.unwrap();
}