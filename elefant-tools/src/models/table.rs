    ...
    pub fn get_create_statement(
        ...
        } = &self.table_type
        {
            ...
            for column in &self.columns {
                ...
                if let Some(identity) = &column.identity {
                    match identity {
                        ColumnIdentity::GeneratedAlways => sql.push_str(" generated always as identity"),
                        ColumnIdentity::GeneratedByDefault => sql.push_str(" generated by default as identity")
                    }

                    match (column.sequence_start, column.sequence_increment) {
                        (Some(start), Some(increment)) => {
                            if start != 1 || increment != 1 {
                                ...
                                sql.push_str(&increment.to_string());
                                sql.push(')');
                            }
                        }
                        _ => {}
        ...
        for col in &self.columns {
            if col.identity.is_some() {
                if let Some(last_value) = col.sequence_last_value {
                    sql.push_str("\nselect setval(pg_get_serial_sequence('");
                    sql.push_str(&escaped_relation_name);
                    sql.push_str("', '");
                    sql.push_str(&col.name);
                    sql.push_str("'), ");
                    sql.push_str(&last_value.to_string());
                    sql.push_str(", true);");
                }
            }
        }
