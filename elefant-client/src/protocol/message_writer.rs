...
use crate::protocol::messages::{
    BackendMessage, Bind, CloseType, CopyResponse, DescribeTarget, ErrorResponse, FrontendMessage,
};
...
use crate::protocol::{
    ...
    AuthenticationSASLContinue, AuthenticationSASLFinal, BackendKeyData, CancelRequest, Close,
    CommandComplete, CopyData, CopyFail, CurrentTransactionStatus, DataRow, Describe, Execute,
    FrontendPMessage, FunctionCall, FunctionCallResponse, NegotiateProtocolVersion,
    NotificationResponse, ParameterDescription, ParameterStatus, Parse, Query, ReadyForQuery,
    RowDescription, StartupMessage, ValueFormat,
};
...
impl<'a, 'b> PostgresMessageEncoder<'a, 'b> {
    fn encode_backend_message(
        mut self,
        message: &BackendMessage<'_>,
    ) -> Result<(), std::io::Error> {
        match message {
            BackendMessage::AuthenticationOk => self.write_authentication_ok(),
            BackendMessage::AuthenticationKerberosV5 => self.write_authentication_kerberos_v5(),
            ...
            BackendMessage::AuthenticationGSS => self.write_authentication_gss(),
            BackendMessage::AuthenticationGSSContinue(gss) => {
                self.write_authentication_gss_continue(gss)
            }
            BackendMessage::AuthenticationSSPI => self.write_authentication_sspi(),
            BackendMessage::AuthenticationSASL(sasl) => self.write_authentication_sasl(sasl),
            ...
            BackendMessage::BackendKeyData(bkd) => self.write_backend_key_data(bkd),
            BackendMessage::BindComplete => self.write_bind_complete(),
            BackendMessage::CloseComplete => self.write_close_complete(),
            BackendMessage::CommandComplete(cc) => self.write_command_complete(cc),
            BackendMessage::CopyData(cd) => self.write_copy_data(cd),
            BackendMessage::CopyDone => self.write_copy_done(),
            BackendMessage::CopyInResponse(cr) => self.write_copy_in_response(cr),
            BackendMessage::CopyOutResponse(cr) => self.write_copy_out_response(cr),
            BackendMessage::CopyBothResponse(cr) => self.write_copy_both_response(cr),
            BackendMessage::DataRow(dr) => self.write_data_row(dr),
            BackendMessage::EmptyQueryResponse => self.write_empty_query_response(),
            BackendMessage::ErrorResponse(er) => self.write_error_response(er),
            BackendMessage::NoticeResponse(er) => self.write_notice_response(er),
            BackendMessage::FunctionCallResponse(fr) => self.write_function_call_response(fr),
            BackendMessage::NegotiateProtocolVersion(npv) => {
                self.write_negotiate_protocol_version(npv)
            }
            BackendMessage::NoData => self.write_no_data(),
            BackendMessage::NotificationResponse(nr) => self.write_notification_response(nr),
            BackendMessage::ParameterDescription(dp) => self.write_parameter_description(dp),
            BackendMessage::ParameterStatus(ps) => self.write_parameter_status(ps),
            BackendMessage::ParseComplete => self.write_parse_complete(),
            BackendMessage::PortalSuspended => self.write_portal_suspended(),
            BackendMessage::ReadyForQuery(q) => self.write_ready_for_query(q),
            BackendMessage::RowDescription(rd) => self.write_row_description(rd),
        }
    ...
    fn write_ready_for_query(self, q: &ReadyForQuery) {
        ...
        self.destination
            .write_u8(match q.current_transaction_status {
    ...
    fn write_negotiate_protocol_version(&mut self, npv: &NegotiateProtocolVersion<'_>) {
        ...
        self.destination
            .write_i32(npv.protocol_options.len() as i32);
    ...
    fn write_copy_response(self, cir: &CopyResponse) {
        ...
        self.destination.write_u8(match cir.format {
            ValueFormat::Text => 0,
            ValueFormat::Binary => 1,
        });
    ...
    fn encode_frontend_message(mut self, message: &FrontendMessage<'_>) {
        match message {
            ...
            FrontendMessage::CancelRequest(cr) => self.write_cancel_request(cr),
            FrontendMessage::Close(close) => self.write_close(close),
            FrontendMessage::CopyData(cd) => self.write_copy_data(cd),
            FrontendMessage::CopyDone => self.write_copy_done(),
            FrontendMessage::CopyFail(cf) => self.write_copy_fail(cf),
            FrontendMessage::Describe(d) => self.write_describe(d),
            FrontendMessage::Execute(e) => self.write_execute(e),
            FrontendMessage::Flush => self.write_flush(),
            FrontendMessage::FunctionCall(fc) => self.write_function_call(fc),
            FrontendMessage::GSSENCRequest => self.write_gss_enc_request(),
            FrontendMessage::FrontendPMessage(gr) => self.write_frontend_p_message(gr),
            FrontendMessage::Parse(p) => self.write_parse(p),
            FrontendMessage::Query(q) => self.write_query(q),
            FrontendMessage::SSLRequest => self.write_ssl_request(),
            FrontendMessage::StartupMessage(s) => self.write_startup_message(s),
            FrontendMessage::Sync => self.write_sync(),
            FrontendMessage::Terminate => self.write_terminate(),
        }
    }
    ...
    fn write_startup_message(&mut self, s: &StartupMessage<'_>) {
        let length = 4
            ...
            + s.parameters
                .iter()
                .map(|p| p.name.len() + 1 + p.value.len() + 1)
                .sum::<usize>() as i32
            + 1;
        self.destination.write_i32(length);
    ...
    fn write_parse(&mut self, p: &Parse<'_>) {
        ...
        let length =
            4 + p.destination.len() + 1 + p.query.len() + 1 + 2 + p.parameter_types.len() * 4;
        self.destination.write_i32(length as i32);
        self.destination
            .write_null_terminated_string(&p.destination);
    ...
    fn write_function_call(&mut self, fc: &FunctionCall<'_>) {
        ...
        let length = 4
            + 4
            + 2
            + (fc.argument_formats.len() as i32 * 2)
            + 2
            + (fc
                .arguments
                .iter()
                .map(|a| if let Some(a) = a { a.len() as i32 } else { 0 } + 4)
                .sum::<i32>())
            + 2;
        self.destination.write_i32(length);
    ...
    fn write_copy_fail(&mut self, cf: &CopyFail<'_>) {
        self.destination.write_u8(b'f');
        self.destination.write_i32(4 + cf.message.len() as i32 + 1);
    ...
    fn write_close(&mut self, close: &Close<'_>) {
        ...
        self.destination.write_u8(match close.target {
    ...
    fn write_bind_message(&mut self, bind: &Bind<'_>) {
        ...
        self.destination.write_i32(length as i32);
        self.destination
            .write_bytes(bind.destination_portal_name.as_bytes());
        self.destination.write_u8(0);
        self.destination
            .write_bytes(bind.source_statement_name.as_bytes());
        self.destination.write_u8(0);
        self.destination
...
impl<C: ElefantAsyncReadWrite> PostgresConnection<C> {
    pub async fn write_backend_message(
        ...
        self.connection
            .write_frame::<PostgresMessageEncoder, _>(message)
            .await
    }
